flm.ce.intspols <- function(d, formula, npf, t, n, dx, knotsx, pars, db, knotsb, v, nx,
  ...
  for (h in (npf+1):(npf+no.pols)) {
    ...
    if (pars[h] == "power") {
      jmat[[h]] <- bsalljcbpowercpp(t=t, j=0, nbs=lkxi, aposx=aposx, alldeg=c(rep(dx[dxpols.coord], dx.pols[dxpols.coord])), allknotsx=allknotsxb, nx=nx.ints, nb=nb[h])
    } else {
  ...
  if (no.ints > 0) {
    for (q in (npf+no.pols+1):(npf+no.pols+no.ints)) {
      ...
      aposx <- as.matrix(expand.grid(lnxvec))
      if (pars[q] == "power") {
        jmat[[q]] <- bsalljcbpowercpp(t=t, j=0, nbs=lkxi, aposx=aposx, alldeg=dx[dx.ints==1], allknotsx=allknotsxb, nx=nx.ints, nb=nb[q])
      } else {
